...
// ── Pressure metric ───────────────────────────────────────────────────────────
// Pressure(Team) = 200 * ((Tackles[team] * 2) + Turnovers[opp]) / EffDisposals[opp].
...
function pressureVal(acc, team, opp) {
  ...
  const num = 200 * ((acc[team].T * 2) + acc[opp].TO);
...
// rolling 10-minute window. Returns raw windowed I50 count and In50s-per-scoring-
// shot per team; the frontend renders the territory bars (threshold 2.20).
...
function computeBursts(fetchLog) {
  ...
  for (const [key, { name, team, series }] of playerMap) {
    ...
    // Greedily pick non-overlapping windows for this player
    const used = [];
